...
//        return "abc";
    ...
    *   解决：可以使用向下转型（强转）把Object类型转换为Person类型
    * */
    /*@Override
    public boolean equals(Object obj) {
        // 增加一个判断，如果传递的参数obj是this本身，直接返回true，提高程序的效率
        if (obj == this) {
            return true;
        }
        ...
        if (obj instanceof Person) {
            // 使用向下转型（强转）把Object类型转换为Person类型
            Person p = (Person) obj;
            // 比较两个对象的属性：一个是调用方法的this(p1)，一个就是p(obj=p2)
            // name是String类型，所以不能直接比较，要用重写的equals方法
            boolean b = this.name.equals(p.name) && this.age == p.age;
            return b;
        }
        // 不是Person类型直接返回值false
        return false;
    }*/
